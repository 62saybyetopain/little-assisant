...
// 實作資料回填介面
  ...
  loadFromData(bodyParts = [], muscleTags = [], assessmentResults = []) {
      ...
      // 強制將輸入轉換為陣列，防止 null 或 legacy string 導致崩潰
      const safeBodyParts = Array.isArray(bodyParts) ? bodyParts : [];
      const safeMuscleTags = Array.isArray(muscleTags) ? muscleTags : [];
      const safeResults = Array.isArray(assessmentResults) ? assessmentResults : [];

      // 1. 還原人體圖選取
      this.bodyDiagram.clearSelection(); 
      if (safeBodyParts.length > 0) {
          safeBodyParts.forEach(partId => {
             if (!partId) return;
      ...
      if (this.muscleTagSelector) {
          ...
          this.muscleTagSelector.updateForBodyParts(safeBodyParts);
          
          // 立即執行勾選 (因為 updateForBodyParts 是同步渲染 innerHTML)
          this.muscleTagSelector.clearSelection();
          
          if (safeMuscleTags.length > 0) {
              safeMuscleTags.forEach(tagId => {
      ...
      if (this.assessmentSelector) {
          ...
          this.assessmentSelector.updateForBodyParts(safeBodyParts).then(() => {
